...
export async function POST(req: Request) {
  ...
  const { data: source } = await admin
    .from("scrape_sources")
    .select("id,url,list_container_selector,link_selector,content_selector,ignore_selector")
  ...
  containers.each((_idx, el) => {
    ...
    const title = a.text().trim() || $(el).text().trim().slice(0, 120);
  ...
  const previews: Array<{ url: string; title: string; contentSnippet: string }> = [];

  for (const item of items) {
    const pageRes = await fetchHtml(item.url);
    if (!pageRes.ok) {
      previews.push({ url: item.url, title: item.title, contentSnippet: "" });
      continue;
    }
    ...
    previews.push({
      url: item.url,
      title: item.title,
      contentSnippet: text.length > 280 ? `${text.slice(0, 280)}…` : text,
    });
  }

  return Response.json({ items: previews }, { status: 200 });
}
